...
function AssetBody({ group, asset }: { group: AssetGroup; asset: CapabilityAsset }) {
  if (group === "skills")
    return (
      <>
        ...
        <details>
          <summary>查看 SKILL.md 指令与文件</summary>
          <pre>{asset.instructions_markdown}</pre>
          <p className="assetFiles">{asset.files?.map((file) => file.path).join(" · ")}</p>
        </details>
      </>
    );
  if (group === "policies")
    return (
      <div className="commitmentList">
        {asset.requirements?.commitments?.map((node) => (
          <div key={node.id}>
  ...
  return (
    <>
      ...
      <ul className="knowledgeObservations">
        {asset.content?.observations?.map((item) => (
          <li key={item}>{item}</li>
        ))}
      </ul>
      <dl className="knowledgeSource">
        <div>
          <dt>来源</dt>
          <dd>
            {sourceType}
            {asset.source?.case_id ? ` · ${asset.source.case_id}` : ""}
          </dd>
        </div>
        <div>
          <dt>观察日期</dt>
          <dd>{asset.source?.observed_at ?? "—"}</dd>
        </div>
        <div>
          <dt>审核</dt>
          <dd>{asset.source?.reviewed_by ?? "—"}</dd>
        </div>
        <div>
          <dt>置信度</dt>
          <dd>{confidence ?? "—"}</dd>
...
function AssetCard({
  ...
  const skillKind =
    asset.kind === "bundle" || (asset.members && asset.members.length > 0)
      ? "SKILL BUNDLE"
      : "ATOMIC SKILL";
  return (
    <article
      className={`assetCard${level ? ` skill-${level.toLowerCase().replace(" ", "-")}` : ""}`}
    >
      <header>
        <div className={`assetKind kind-${group}`}>
          {group === "skills" ? "S" : group === "policies" ? "P" : "K"}
        </div>
        <div>
          {group === "skills" && <span className="assetLevel">{level ?? skillKind}</span>}
          <h2>{asset.title}</h2>
          <p>
            {asset.id} · v{asset.resolved_ref.version}
          </p>
        </div>
        <span className={`assetOrigin ${asset.resolved_ref.source}`}>
          {asset.resolved_ref.source === "local" ? "本地覆盖" : "内置"}
        </span>
      </header>
      {group === "skills" ? (
        <p className="assetSelector">
          维护角色 · {asset.maintainer_role?.trim() || "平台公共能力"}
        </p>
      ) : (
      ...
      <footer>
        <span>SHA-256</span>
        <code>{asset.resolved_ref.digest.replace("sha256:", "").slice(0, 16)}</code>
      </footer>
    </article>
  );
}
...
function assetMatches(asset: CapabilityAsset, keyword: string) {
  return (
    !keyword ||
    [asset.title, asset.id, asset.description, asset.content?.summary].some((value) =>
...
function skillMatchesSearch(
  skill: CapabilityAsset,
  keyword: string,
  byId: Map<string, CapabilityAsset>,
) {
  ...
  return (skill.members ?? []).some((memberId) => {
    ...
    return Boolean(
      member &&
        (assetMatches(member, keyword) || skillRole(member).toLowerCase().includes(keyword)),
    );
  });
}
...
function SkillRoleLibrary({ skills, search }: { skills: CapabilityAsset[]; search: string }) {
  ...
  const namedRoles = [...roleGroups.keys()]
    .filter((role) => role !== publicRole)
  ...
  return (
    <>
      {orderedRoles.map((role) => {
        const roleSkills = [...(roleGroups.get(role) ?? [])].sort((left, right) =>
          left.title.localeCompare(right.title, "zh"),
        );
        return (
          <section className="skillRoleGroup" key={role}>
            <header className="skillRoleHeader">
              <div>
                <span className="assetLevel">MAINTAINER ROLE</span>
                <h2>{role}</h2>
              </div>
              <small>{roleSkills.length} 项</small>
            </header>
            <div className="skillRoleGrid">
              {roleSkills.map((skill) => {
                const members = (skill.members ?? [])
                  .map((memberId) => byId.get(memberId))
                  .filter((member): member is CapabilityAsset => Boolean(member));
                return (
                  <AssetCard
                    key={skill.id}
                    group="skills"
                    asset={skill}
                    level={
                      skill.kind === "bundle" || members.length > 0
                        ? "SKILL BUNDLE"
                        : "ATOMIC SKILL"
                    }
                  >
...
export default function AssetLibrary({ group }: { group: AssetGroup }) {
  ...
  useEffect(() => {
    ...
    apiGet<LibraryResponse>("/api/capabilities", undefined, controller.signal)
      .then(setData)
      .catch((reason) => {
        if (!isAbort(reason)) setError(true);
      });
    return () => controller.abort();
  }, []);
  ...
  const skillById = useMemo(
    () => new Map((data?.assets.skills ?? []).map((skill) => [skill.id, skill])),
    [data],
  );
  const skillHasMatch =
    group === "skills" &&
    ...
    <div className="appShell">
      ...
      <main className="mainArea">
        ...
        <div className="assetPage">
          <section className="assetHero">
            ...
            <label className="assetSearch">
              ...
              <input
                ...
                placeholder={`搜索 ${copy.title}`}
                aria-label={`搜索 ${copy.title}`}
              />
            </label>
          </section>
          {data && (
            <section className="assetStats" aria-label="组织资产统计">
              <a className={group === "skills" ? "active" : ""} href="/assets/skills">
                <span>Skills</span>
                <strong>{data.counts.skills}</strong>
              </a>
              <a className={group === "policies" ? "active" : ""} href="/assets/policies">
                <span>Policies</span>
                <strong>{data.counts.policies}</strong>
              </a>
              <a className={group === "knowledge" ? "active" : ""} href="/assets/knowledge">
                <span>Knowledge</span>
                <strong>{data.counts.knowledge}</strong>
              </a>
            </section>
          )}
          {error && (
            <div className="assetMessage error">
              <strong>无法读取后台资产</strong>
              <p>请确认 Agentic CM API 已启动并可从当前页面访问。</p>
            </div>
          )}
          {!data && !error && (
          ...
          {data && isEmpty && (
            <div className="assetMessage">
              <strong>{search ? "没有匹配的资产" : copy.empty}</strong>
              <p>
                {search ? "请尝试其他名称、ID、Role 或描述关键词。" : "发布资产后刷新即可显示。"}
              </p>
            </div>
          )}
          <section
            className={group === "skills" ? "skillRoleLibrary" : "assetGrid"}
            aria-live="polite"
          >
            {group === "skills" && data && (
              <SkillRoleLibrary skills={data.assets.skills} search={search} />
            )}
            {group !== "skills" &&
              assets.map((asset) => (
                <AssetCard group={group} asset={asset} key={asset.resolved_ref.id} />
              ))}
          </section>
        </div>
      </main>
