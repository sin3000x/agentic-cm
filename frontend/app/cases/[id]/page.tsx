...
export default function Home() {
  ...
  const startAutomaticAlternatives = useEffectEvent((pathIds: string[]) => {
    void generateAlternatives(pathIds);
  });
  const startAutomaticSynthesis = useEffectEvent(() => {
    void generateSynthesis();
  });
  const refreshLiveTrace = useEffectEvent(() => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([data, timeline]) => {
        ...
        setApproved(
          ["PATH_EXPLORATION", "PROFESSIONAL_COMMITMENT", "FINAL_REVIEW"].includes(data.phase),
        );
        ...
        if (["PATH_EXPLORATION", "PROFESSIONAL_COMMITMENT", "FINAL_REVIEW"].includes(data.phase)) {
          setSelectedPathIds(
            (data.manifest?.paths ?? []).filter((path) => path.selected).map((path) => path.id),
          );
        }
        if (data.permissions?.can_view_manifest === true) {
          Promise.all(
            ["orchestrator", "path", "synthesis"].map((agentType) =>
              apiGet<AgentRun[]>(
        ...
        if (
          data.permissions?.can_view_manifest === true &&
          data.phase === "INTAKE" &&
          !data.manifest
        ) {
        ...
        if (
          data.permissions?.can_decide_case === true &&
          data.phase === "FINAL_REVIEW" &&
          !data.synthesis_report
        ) {
  ...
  async function refreshAgentRuns() {
    ...
    try {
      ...
      const loadRuns = (agentType: string) =>
        apiGet<AgentRun[]>(`/api/cases/${activeCaseId}/agent-runs`, {
          ...baseQuery,
  ...
  async function generateManifest() {
    ...
    setMessage("");
    try {
      const data = await apiPost<CaseDetails>(`/api/cases/${activeCaseId}/orchestrate`, {
      ...
      setMessage(
        `Manifest 生成失败：${error instanceof Error ? error.message : "请确认本地 API 与 Planner 配置"}。`,
      );
    } finally {
  ...
  async function generateAlternatives(pathIds: string | string[]) {
    ...
    setMessage("");
    try {
      const data = await apiPost<PathExecutionResponse>(
        `/api/cases/${activeCaseId}/paths/execute`,
        {
      ...
      setApproved(
        ["PATH_EXPLORATION", "PROFESSIONAL_COMMITMENT", "FINAL_REVIEW"].includes(updatedCase.phase),
      );
      await refreshAgentRuns();
      await refreshTimeline();
      setMessage(
      ...
      setMessage(
        `替代方案生成失败：${error instanceof Error ? error.message : "请确认 Path Agent 配置"}。`,
      );
    } finally {
      await refreshAgentRuns();
      setAiRunKind(null);
  ...
  async function generateSynthesis() {
    ...
    setMessage("");
    try {
      const data = await apiPost<CaseDetails>(`/api/cases/${activeCaseId}/synthesize`, {
        actor: currentIdentity.name,
        role: currentIdentity.role,
      });
      setSynthesisReport(data.synthesis_report ?? null);
      ...
      setMessage(
        `汇总报告生成失败：${error instanceof Error ? error.message : "请确认 Synthesis Agent 配置"}。`,
      );
    } finally {
      await refreshAgentRuns();
      setAiRunKind(null);
  ...
  async function decideCase(action: OwnerDecision["action"], guidance?: string) {
    ...
    try {
      ...
      setMessage(
        `最终决策失败：${error instanceof Error ? error.message : "请确认当前身份与 Case 状态"}。`,
      );
    } finally {
      setBusy(false);
    }
  ...
  async function decideCommitment(
    caseId: string,
    node: CommitmentNode,
  ...
  const selectedPathViews = manifestPaths
    ...
    .map((path) => {
      const attempt = pathAttempts.find((item) => item.path_id === path.id);
      const revision = isSolutionRevision(attempt?.solution_revision)
        ? attempt.solution_revision
        : null;
      const nodes = commitmentNodes.filter((node) => node.path_id === path.id);
      const runs = pathAgentRuns.filter((run) =>
        run.events.some(
  ...
  const liveAgentType =
    aiRunKind === "manifest" ? "orchestrator" : aiRunKind === "alternatives" ? "path" : "synthesis";
  const allLiveAgentRuns =
    liveAgentType === "orchestrator"
      ? agentRuns
      : liveAgentType === "path"
        ? pathAgentRuns
        : synthesisAgentRuns;
  ...
  const latestFailedRunCount =
    latestFailedOrchestratorRuns.length +
    latestFailedPathRuns.length +
    latestFailedSynthesisRuns.length;
  const hasApprovedManifest =
    canViewManifest &&
  ...
  function commitmentNode(
    ...
    const statusLabel =
      node.status === "PENDING"
        ? node.role === currentIdentity.role
          ? "待本人批准"
          : `待${node.role}批准`
        : node.status === "BLOCKED"
          ? "等待前置审批"
  ...
  const orchestrationCard = aiRunKind ? (
    <AiWorkingCard
      ...
      runs={liveAgentRuns}
      paths={
        aiRunKind === "alternatives"
          ? manifestPaths.filter((path) => aiPathIds.includes(path.id))
          : []
      }
    ...
    <>
      <div className="panelTitle">
        <div>
          ...
          <span>
            <small>OWNER-ONLY REVIEW</small>
            <h2>Manifest 正在等待 Case Owner 审批</h2>
          </span>
        </div>
    ...
    <>
      <div className="panelTitle">
        <div>
          <span className="agentIcon">◇</span>
          <span>
            <small>PATH EXPLORATION</small>
            <h2>探索已批准的 Path</h2>
          </span>
        </div>
        <span className="version">
          {completedExplorationCount} / {manifestPaths.filter((path) => path.selected).length} READY
        </span>
      </div>
      <p className="lead">
    ...
    <>
      ...
      <div className="pathApprovalList">
        {selectedPathViews.map(({ path, revision, nodes, runs }) => {
          ...
          return (
            <section
              className="pathApprovalGroup"
              aria-label={`${path.title} 审批 DAG`}
              key={path.id}
            >
              <header className="pathApprovalHeader">
                ...
                <em>
                  {completedNodes} / {nodes.length} 已通过
                </em>
              </header>
              {revision ? (
                <section
                  className="solutionRevision sharedSolution"
                  aria-label={`${path.title} 推荐方案`}
                >
                  <div className="solutionHeader">
                    <span>
                      <small>PATH RECOMMENDATION</small>
                      <strong>v{revision.revision} · 推荐方案</strong>
                    </span>
      ...
      <div className="metricStrip">
        <span>
          <strong>{selectedPathViews.length}</strong>
          <small>selected Paths</small>
        </span>
        <span>
          <strong>{commitmentNodes.length}</strong>
          <small>commitment nodes</small>
        </span>
        <span>
          <strong>{commitmentNodes.filter((node) => node.status === "PENDING").length}</strong>
      ...
      {canViewManifest && (
        <>
          <button className="linkButton capabilityToggle" onClick={toggleCapabilities}>
            {showCapabilities ? "收起能力快照 ↑" : "查看本次能力快照 →"}
          </button>
    ...
    <>
      <div className="panelTitle">
        <div>
          <span className="agentIcon">∑</span>
          <span>
            <small>CASE SYNTHESIS</small>
            <h2>全 Path 汇总 · Owner 决策</h2>
          </span>
        </div>
      ...
      {!canViewManifest ? (
        <p className="lead">
          {isCaseClosed
            ...
            : "所有 Path 的审批 DAG 已完成。汇总报告与最终决策仅对 Case Owner 可见。"}
        </p>
      ) : synthesisReport ? (
        <section className="synthesisReport" aria-label="Synthesis Agent 汇总报告">
          <div className="synthesisHeader">
            <span>
              <small>SYNTHESIS REPORT</small>
              <strong>{synthesisReport.summary}</strong>
            </span>
              ...
              return (
                <article className={assessment.status.toLowerCase()} key={assessment.path_id}>
                  ...
                  {assessment.risks.length > 0 && (
                    <small>风险：{assessment.risks.join("；")}</small>
                  )}
                </article>
              );
            })}
          ...
          <div className="synthesisFindings">
            <div>
              <strong>跨 Path 结论</strong>
              <p>{synthesisReport.cross_path_findings.join("；") || "无新增结论"}</p>
            </div>
            <div>
              <strong>剩余风险</strong>
              <p>{synthesisReport.remaining_risks.join("；") || "未记录剩余风险"}</p>
            </div>
          </div>
          <div className="ownerDecisionBrief">
            <span>
              <small>AGENT 建议 · 非最终决定</small>
              <strong>{synthesisReport.recommended_owner_action}</strong>
            </span>
            <p>{synthesisReport.decision_brief}</p>
          </div>
          {ownerDecision && (
            <p className="decisionRecorded">
              最近决定：{ownerDecision.action} · {formatThreadTime(ownerDecision.decided_at)}
            </p>
          )}
          <div className="ownerDecisionActions" aria-label="Case Owner 最终决策">
            <button
              className="decisionClose"
              disabled={busy || caseStatus === "CLOSED"}
              onClick={() => decideCase("CLOSE")}
            >
              关闭 Case
            </button>
            <button
              className="decisionOpen"
              disabled={busy || caseStatus === "CLOSED"}
              onClick={() => decideCase("KEEP_OPEN")}
            >
              保持 Open
            </button>
            <button
              className="decisionModify"
              disabled={busy || caseStatus === "CLOSED"}
              onClick={() => setShowModifyGuidance((current) => !current)}
            >
              修改方案
            </button>
          </div>
          {showModifyGuidance && caseStatus !== "CLOSED" && (
            <div className="modifyGuidance" aria-label="给 Orchestrator 的修改指导">
              ...
              <div>
                ...
                <button
                  className="primary"
                  disabled={busy || !modifyGuidance.trim()}
                  onClick={() => decideCase("MODIFY", modifyGuidance)}
                >
                  提交指导并重新编排
                </button>
              </div>
          ...
          {synthesisAgentRuns.length > 0 && (
            <>
              <button
                className="linkButton traceToggle"
                onClick={() => setShowSynthesisTrace((current) => !current)}
              >
                {showSynthesisTrace
                  ? "收起 Synthesis Trace ↑"
                  : `查看 Synthesis Trace (${synthesisAgentRuns.length}) →`}
              </button>
              {showSynthesisTrace && (
                <AgentTracePanel runs={synthesisAgentRuns} agentType="synthesis" />
              )}
            </>
          )}
        </section>
              ...
              <div className="manifestSkillChoices">
                {path.skill_selections.map((selection) => (
                  <details className="manifestSkillChoice" key={selection.entrypoint.id}>
                    ...
                    <p>
                      <b>选择理由</b>
                      {selection.reason ?? "历史 Manifest 未记录选择理由"}
                    </p>
                    {selection.members.length > 0 && (
                      <div>
                        <b>Bundle 成员</b>
                        {selection.members.map((member) => (
                          <code key={member.id}>{skillLabel(member)}</code>
                        ))}
                      </div>
                    )}
                  </details>
          ...
          <header className="issueHeader">
            <div>
              <div className="eyebrow">
                SUPPLY CHAIN CASE <span>·</span>{" "}
                {caseDetails?.business_payload?.risk_level === "HIGH"
                  ? "高"
                  : caseDetails?.business_payload?.risk_level === "LOW"
                    ? "低"
                    : "中"}
                优先级
              </div>
              <h1>
                {caseDetails?.title ?? "正在加载 Case"}{" "}
                <span>#{caseDetails?.id ?? activeCaseId}</span>
              </h1>
              <p>
                <span className={`openBadge ${caseStatus.toLowerCase()}`}>
                  ● {caseStatus === "CLOSED" ? "Closed" : "Open"}
                </span>{" "}
                {caseDetails?.owner ?? "Case Owner"} 于 {formatThreadTime(caseCreatedAt)} 创建 ·
                ...
                <div className="futureFlow" aria-label="后续流程">
                  {activeStageIndex < 3 && (
                    <div>
                      <span>4</span>
                      <p>
                        <strong>专业承诺汇合</strong>
                        <small>Path 探索完成后开放；供应与技术并行评审</small>
                      </p>
                    </div>
                  )}
                  <div>
                    <span>5</span>
                    <p>
                      <strong>Case Owner 最终决策</strong>
                      <small>基于 Synthesis 报告选择关闭、保持 Open 或打回修改</small>
                    </p>
                  </div>
                </div>
              )}
            ...
            <aside className="rightRail">
              <section className="panel flowSummary">
                ...
                <ol>
                  {stages.map((stage, index) => (
                    <li
                      className={
                        index < activeStageIndex || (isCaseClosed && index === activeStageIndex)
                          ? "complete"
              ...
              <section className="panel facts">
                ...
                <dl>
                  <div>
                    <dt>Case Owner</dt>
                    <dd>{caseDetails?.owner ?? "—"}</dd>
                  </div>
                  <div>
                    <dt>订单</dt>
                    <dd>{caseDetails?.business_payload?.order_id ?? "—"}</dd>
                  </div>
                  <div>
                    <dt>客户</dt>
                    <dd>{caseDetails?.business_payload?.customer ?? "—"}</dd>
                  </div>
                  <div>
                    <dt>关键物料</dt>
                    <dd>{caseDetails?.business_payload?.material ?? "—"}</dd>
                  </div>
                  <div>
                    <dt>缺口数量</dt>
                    <dd>
                      {caseDetails?.business_payload?.gap_quantity !== undefined
                        ? `${formatQuantity(caseDetails.business_payload.gap_quantity)} pcs`
                        : "—"}
                    </dd>
                  </div>
                  <div>
                    <dt>目标交付日</dt>
                    <dd>{caseDetails?.business_payload?.target_date ?? "—"}</dd>
                  </div>
                </dl>
              </section>
